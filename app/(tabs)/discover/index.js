...
// Kaşif (orta sekme) — Google Maps + POI pinleri + LLM rota + beraber gez
// DB: pois (GIST bbox), routes (LLM), active_explorers (TTL 5dk), events, live_sessions
// LLM: composite_score = %30 popülerlik + %20 kalite + %15 sponsor + %20 zaman + %15 tercih

import React, { useState, useRef, useCallback, useEffect } from 'react';
import { StyleSheet, Text, View, TextInput, TouchableOpacity,
  ScrollView, Animated, Dimensions, ActivityIndicator, Alert, Platform } from 'react-native';
...
// POI kategorisi → pin rengi (pois.category → renk)
...
// Rota türü (transport_mode ENUM)
const TRANSPORT_LABELS = {
  walking: '🚶 Yürüyüş', cycling: '🚴 Bisiklet',
  driving: '🚗 Araç',    transit: '🚌 Toplu Taşıma',
};
...
export default function DiscoverScreen() {
  ...
  // cityId: konum tespit edildiğinde detect-city ile bulunacak
  ...
  useEffect(() => {
    if (location) {
      loadNearbyData(location.coords.latitude, location.coords.longitude);
    }
  }, [location?.coords.latitude]);

  // LLM rota gelince sheet aç
  useEffect(() => {
    if (route) openSheet();
  }, [route]);
  ...
  // Harita kaydırınca BBox'taki POI'ları yükle
  const onRegionChangeComplete = useCallback((region) => {
    loadPOIsForBBox(region);
  }, [loadPOIsForBBox]);

  // Aktif gezgin durumu — active_explorers UPSERT (expires_at=+5dk)
  ...
  return (
    <View style={styles.container}>
      {/* Google Maps — POI pinleri + rota polyline */}
      <MapView
        ref={mapRef}
      ...
      >
        {/* POI pinleri — pois.location (GEOMETRY Point 4326) */}
        {pois.map(poi => (
          <Marker
            key={poi.id}
        ...
        {/* Aktif gezginler — active_explorers (expires_at filtreli) */}
        ...
        {/* LLM Rota polyline — routes.geometry (LineString) */}
        ...
        {route?.stops?.map((s,i) => s.lat && (
          <Marker key={i} coordinate={{ latitude: s.lat, longitude: s.lng }}>
            <View style={[styles.routePin, { backgroundColor: i===0?'#22C55E': i===route.stops.length-1?'#2196F3':'#FF9800' }]}>
      ...
      {/* Sağ üst: hızlı erişim butonları */}
      <View style={[styles.rightBtns, { top: insets.top + 140 }]}>
        {/* Beraber gez — active_explorers */}
        <TouchableOpacity
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY: sheetTranslateY }] }]}>
          ...
          <View style={styles.sheetBadges}>
            <View style={styles.badge}><Text style={styles.badgeText}>
              {route.total_hours ? `${route.total_hours} saat` : ''}
            </Text></View>
            <View style={styles.badge}><Text style={styles.badgeText}>
          ...
          {/* Durak listesi — route_stops (suggested_time, est_duration_min DB'den) */}
