  ...
  return (
    <View style={{ marginTop: 10 }}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <Image source={{ uri: item.storage_path || item.url }} style={{ width, height: width * 0.75, resizeMode: 'cover' }} />
        )}
      />
  ...
  return (
    <View style={s.card}>
      <View style={s.cardHeader}>
        <TouchableOpacity style={s.cardUser} onPress={() => router.push(`/user/${item.user_id}`)} activeOpacity={0.7}>
        ...
        <View style={s.actionLeft}>
          <TouchableOpacity onPress={handleLike} style={s.actionBtn}>
            <Animated.View style={{ transform: [{ scale: likeAnim }] }}>
              <Ionicons name={item.liked ? 'heart' : 'heart-outline'} size={24} color={item.liked ? '#ef4444' : '#888'} />
            </Animated.View>
